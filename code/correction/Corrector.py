...
class Corrector():
    ...
    def trigram_path_probability(self, path, lower=True):
       ...
       if lower:
           word1, word2, word3 = [w.lower() for w in ['<s>', '<s>'] + path[-3:]][-3:]
       else:
           word1, word2, word3 = [w for w in ['<s>', '<s>'] + path[-3:]][-3:]
    ...
    def get_correction(self, tokens):
        ...
        return beam_search(tokens, self.width, self.get_error_prob, self.trigram_path_probability, self.variation_generator, self.verbose)
